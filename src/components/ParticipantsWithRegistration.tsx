...
// Mock registrations for the selector
const mockRegistrations = [
  { id: '1', name: 'Summer Adventure Camp 2024' },
  { id: '2', name: 'Winter Sports Camp 2024' },
  { id: '3', name: 'Spring Nature Camp 2024' }
]

export function Participants() {
  const { selectedRegistration, setSelectedRegistration } = useRegistration()
  ...
  // Filter participants by selected registration
  const registrationFilteredParticipants = selectedRegistration 
    ? participants.filter(p => p.registrationId === selectedRegistration.id)
    : participants

  const filteredParticipants = registrationFilteredParticipants.filter(participant => {
    const matchesSearch = 
  ...
  const getStatusBadge = (status: Participant['status']) => {
    const configs = {
      ...
      approved: { variant: 'default' as const, icon: UserCheck, color: 'text-green-700 bg-green-100' },
      rejected: { variant: 'destructive' as const, icon: XCircle, color: 'text-red-700 bg-red-100' },
  ...
  const getStats = () => {
    return {
      ...
      approved: filteredParticipants.filter(p => p.status === 'approved').length,
      rejected: filteredParticipants.filter(p => p.status === 'rejected').length,
      withMedical: filteredParticipants.filter(p => p.medicalConditions && p.medicalConditions.trim()).length
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            Manage camp participant information and registrations
          </p>
        </div>
        <Button 
        ...
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            <div className="flex items-center space-x-2">
              ...
              <label className="text-sm font-medium text-gray-700">Select Registration:</label>
            </div>
            <Select 
              value={selectedRegistration?.id || ''} 
              onValueChange={(value) => {
                const registration = mockRegistrations.find(r => r.id === value)
                setSelectedRegistration(registration ? { ...registration, userId: 'user1', description: '', startDate: '', endDate: '', maxParticipants: 50, currentParticipants: 0, registrationDeadline: '', campFee: 0, status: 'open' as const, createdAt: '', updatedAt: '' } : null)
              }}
            >
              <SelectTrigger className="w-full sm:w-[300px] border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                <SelectValue placeholder="Choose a camp registration" />
              </SelectTrigger>
              <SelectContent>
      ...
      {!selectedRegistration ? (
        <Card className="shadow-lg border-0 bg-white">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Select a Registration</h3>
            <p className="text-gray-600 mb-6">
              Choose a camp registration above to view and manage its participants.
            </p>
            <Button 
              onClick={() => {
          ...
          <Card className="hidden md:block shadow-lg border-0 bg-white">
            <CardHeader className="bg-gradient-to-r from-gray-50 to-slate-50 border-b">
              <CardTitle className="flex items-center">
                ...
                {selectedRegistration.name} - Participants ({filteredParticipants.length})
              </CardTitle>
            </CardHeader>
